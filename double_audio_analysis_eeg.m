...
function [T1,T2] = double_audio_analysis_eeg(data)
...
% -------------------------------------------------------------------------
    ...
    % DATA{3} = Universal mind;  DATA{4} = Adios nonino
    ...
    for k = 1:length(DATA{z})
        for c = 1:size(DATA{z}{k,2},1)
            ...
            create_table_colum_str = sprintf('T1.%s = Variable_aux; ', strcat(DATA{z}{k,1},data.label{c}));
    ...
    for k = 1:length(DATA{z})
        for c = 1:size(DATA{z}{k,2},1)
            ...
            eval(create_data_str); 
            create_table_colum_str = sprintf('T2.%s = Variable_aux; ', strcat(DATA{z}{k,1},data.label{c}));
            eval(create_table_colum_str);
