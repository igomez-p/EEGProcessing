...
function [TBL1,TBL2] = total_audio_analysis_eeg(data)
...
% -------------------------------------------------------------------------
    ...
    for k = 1:NRE
        for c = 1:size(DATA{1}{k,2},1)
            ...
            create_table_colum_str = sprintf('TBL1.%s = Variable_aux; ', strcat(DATA{1}{k,1},data.label{c}));
    ...
    for k = 1:NRE
        for c = 1:size(DATA{1}{k,2},1)
            ...
            eval(create_data_str); 
            create_table_colum_str = sprintf('TBL2.%s = Variable_aux; ', strcat(DATA{1}{k,1},data.label{c}));
            eval(create_table_colum_str);
